"""Website inheritance, real templates and non-exclusive crawler declarations."""
import copy
...
class WebsitePolicyTests(unittest.TestCase):
    ...
    def test_policy_and_crawler_changes_propagate_without_python_edits(self):
        ...
        values = {'title': 'A title', 'slug': 'a-title', 'date': '2026-09-16', 'publication_context': self.context}
        ...
        self.assertIn('User-agent: FutureBot', outputs['robots.txt'])
    ...
    def test_named_and_unknown_crawlers_remain_open(self):
        outputs = render(self.data, self.records)
        parser = RobotFileParser()
        parser.parse(outputs['robots.txt'].splitlines())
        ...
        self.assertIn('User-agent: *', outputs['robots.txt'])
        data = copy.deepcopy(self.data)
    ...
    def test_rsl_site_reference_keeps_exceptions_in_core_terms_without_fees(self):
        ...
        # RSL 'free' would also waive attribution, unlike no-fee CC BY reuse.
    ...
    def test_content_cli_creates_and_refuses_to_overwrite_artifacts(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            values.write_text(yaml.safe_dump({'title': 'Example', 'slug': 'example', 'date': '2026-09-16', 'publication_context': self.context}))
