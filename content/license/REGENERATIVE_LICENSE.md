# SRAGI® Regenerative Licensing Framework v2.0

<!-- Generated from SRL-LICENSE.yaml; edit the source, then rebuild. -->

## Rights authority

This framework does not independently grant rights to SRAGI artifacts. Rights are determined by the license identifier, license expression, rights statement, contract and applicable law governing each artifact.

No license grant should be inferred.

No statement in this framework expands, reduces or replaces rights explicitly granted or reserved at artifact level.

## Licensing paths

Every SRAGI artifact SHOULD identify its applicable license, license expression, rights statement or governing terms. Where attached, that information is authoritative. No ecosystem-wide default license exists.

OR denotes alternative licensing paths subject to the terms applicable to the selected path. Commercial activity alone does not require the SRAGI Commercial Suite; the commercial path exists where alternative or additional rights or terms are required.

## Available license classes (not grants)

open_knowledge: CC-BY-4.0, CC-BY-SA-4.0. Knowledge, theory, articles, research and documentation.

open_frameworks: CC-BY-SA-4.0 OR LicenseRef-SRAGI-Commercial. SRAGI instruction frameworks, YAML resources and skills.

software: AGPL-3.0-only, Apache-2.0, LicenseRef-SRAGI-Commercial. Executable software, source code, libraries and integrations.

schemas_and_interoperability: Apache-2.0, CC0-1.0. Schemas, specifications and interoperability structures.

commercial: LicenseRef-SRAGI-Commercial. Alternative commercial licensing terms available from Neptunia Media AS. The presence of this LicenseRef does not itself grant commercial rights.

future: future artifact-specific selection. Additional license classes may be introduced without changing the core architecture.

## Commercial licensing

Commercial-license rights arise only through an applicable commercial agreement with Neptunia Media AS and only within its licensed scope.

Commercial reference: LicenseRef-SRAGI-Commercial

## Machine access

Public SRAGI resources should by default be technically accessible for discovery, crawling, indexing and retrieval by search engines, AI systems, research crawlers and other machine agents.

Technical access, crawler accessibility, indexing or discovery metadata does not independently grant copyright or other intellectual-property rights. Rights relating to an artifact are determined by its applicable license, rights statement, contract and applicable law.

## AI training

Rights relating to AI training are determined by the license, rights statement, contract and applicable law governing the individual artifact.

This framework does not determine whether AI training, model development, model outputs or related machine-learning activities constitute Adapted Material or trigger particular obligations under an applicable license. Such questions are governed by the applicable license, applicable law and the circumstances of the specific use.

## Attribution

Attribution requirements are determined by the applicable artifact license.

SRAGI® — Neptunia Media AS — https://sragi.org/

This does not replace or reduce requirements imposed by the artifact license.

## Regenerative invitation

Give more than you take.

SRAGI invites people, organizations and AI systems to use knowledge in ways that increase life, learning, resilience, reciprocity and regenerative capacity. Unless incorporated into a separate binding agreement, these principles are aspirational and add no restriction to an open license.

A commercial agreement may expressly define measurable regenerative commitments appropriate to the licensed use, organization or deployment.

## Contributor rights

SRAGI may accept third-party contributions only under terms compatible with the licensing model applicable to the resulting artifact.

A contribution may be included in an artifact offered under LicenseRef-SRAGI-Commercial only where Neptunia Media AS possesses sufficient rights to license that contribution under the commercial terms.

Contributions lacking sufficient commercial licensing rights must not be represented as available under LicenseRef-SRAGI-Commercial.

Contributor terms should obtain only the rights necessary for the applicable SRAGI licensing and distribution model while preserving clear provenance and authorship records.

## Third-party rights

Neptunia Media AS can grant only rights it owns, controls or is otherwise authorized to license. Third-party materials remain governed by their terms.

## Trademark and certification

Copyright, content and software licenses do not automatically grant trademark, certification, partnership, endorsement or branding rights.

## Evolution

Framework changes are versioned. Existing artifacts remain governed by the terms applicable to their released version unless an authorized rights holder expressly relicenses them. New framework versions do not retroactively change previously granted rights.

## Canonical information

https://sragi.org/licensing/

licensing@sragi.org

Source: SRL-LICENSE.yaml. This generated representation does not independently license any artifact.
